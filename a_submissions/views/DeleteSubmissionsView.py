...
class DeleteSubmissionsView(APIView):
    ...
    def delete(self, request, id):      
        ...
        # ensuring the user is  the current writer and he has 0 submissions for that work befoe toggling the work details
        # exp: Can happen when that work is reassigned to another writer and the previous writer deletes their submission. 
        # Doing this enables only the current writer to toggle details when they delete AND HAVE 0 submissions
        if (submission.work.writer==request.user) and (submission.work.submissions.filter(sender=request.user).count()==0):
